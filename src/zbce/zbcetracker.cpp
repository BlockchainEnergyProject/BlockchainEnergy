...
CzBCETracker::CzBCETracker(std::string strWalletFile)
{
...
CzBCETracker::~CzBCETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBCETracker::Init()
{
...
bool CzBCETracker::Archive(CMintMeta& meta)
{
...
bool CzBCETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBCETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBCETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBCETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBCETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBCETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBCETracker::GetSerialHashes()
{
...
CAmount CzBCETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBCETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBCETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBCETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBCETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBCETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBCETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBCETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBCETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBCETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBCETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBCEWallet* zBCEWallet)
{
    bool iszBCEWalletInitialized = (NULL != zBCEWallet);
    CMintMeta meta;
    ...
    if (! iszBCEWalletInitialized)
        zBCEWallet = new CzBCEWallet(strWalletFile);
    meta.isSeedCorrect = zBCEWallet->CheckSeed(dMint);
    if (! iszBCEWalletInitialized)
        delete zBCEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBCETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBCETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBCETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBCETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBCETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBCETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBCEWallet* zBCEWallet = new CzBCEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBCEWallet);
        }
        delete zBCEWallet;
        LogPrint("zero", "%s: added %d dzbce from DB\n", __func__, listDeterministicDB.size());
    }
