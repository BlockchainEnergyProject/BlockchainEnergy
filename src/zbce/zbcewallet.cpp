...
CzBCEWallet::CzBCEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbce seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBCESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBCESeed_deprecated()) {
                    LogPrintf("%s: Updated zBCE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbce seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbce wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBCEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBCECount(nCountLastUsed);
    else if (!walletdb.ReadZBCECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBCEWallet::Lock()
{
    seedMaster = 0;
}

void CzBCEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBCEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBCE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBCEWallet::LoadMintPoolFromDB()
{
...
void CzBCEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBCEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBCEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbceTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBCEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBCE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbceTracker which also adds to database
    pwalletMain->zbceTracker->Add(dMint, true);
...
void CzBCEWallet::SeedToZBCE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBCEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBCEWallet::GenerateDeterministicZBCE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBCEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBCEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBCEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
