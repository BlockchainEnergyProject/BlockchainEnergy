...
/**
 * BlockchainEnergy RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZbceSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzbceseed(const UniValue& params, bool fHelp);
extern UniValue getzbceseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzbce(const UniValue& params, bool fHelp);
extern UniValue dzbcestate(const UniValue& params, bool fHelp);
