...
public:
    ...
    bool WriteZBCESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBCESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBCESeed_deprecated(uint256& seed);
    bool EraseZBCESeed();
    bool EraseZBCESeed_deprecated();

    bool WriteZBCECount(const uint32_t& nCount);
    bool ReadZBCECount(uint32_t& nCount);
