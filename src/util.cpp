...
#else
    const char* pszModule = "blockchainenergy";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "blockchainenergy.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty blockchainenergy.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override blockchainenergy.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "blockchainenergyd.pid"));
