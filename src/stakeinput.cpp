...
// The zBCE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBCE is a hash of the serial
...
{
    //Create an output returning the zBCE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBCEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBCE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBCE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBCEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBCE output", __func__);
...
{
    CzBCETracker* zbceTracker = pwallet->zbceTracker.get();
    CMintMeta meta;
    if (!zbceTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbceTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BCE stake is the outpoint
