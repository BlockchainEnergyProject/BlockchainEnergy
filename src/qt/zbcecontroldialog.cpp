...
#include "ui_zbcecontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbceTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbceTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBCE.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBCE seed used to mint this zBCE is not the same as currently hold in the wallet";
            else
